...
- **Persistent receipt backoff for native sync**: failed native receipts now persist an exponential, jittered retry backoff (1s → 30s, capped at 6 attempts) under the sync store's `retries/` directory, so backoff survives daemon restarts and is no longer bypassed by immediate flushes. Reconnect or capture wakes may still wake healthy work but no longer cancel an in-flight retry timer, and archive flushes are gated while a receipt is backing off.
- **Structured native sync diagnostics**: gateway native-runtime failures now emit full context — event type, space, owner, session, and Turn IDs — alongside the serialized Postgres error surface (`code`, `detail`, `hint`, `constraint`, `table`, `column`, `schema`, `position`) and nested `cause` chains. Operators get actionable sync-failure logs while the client still receives only a generic retry receipt.
...
- Concurrent or replayed native turns no longer overwrite a newer Session title or latest-message summary, and a failing Session projection no longer surfaces as a native sync error.
- Failed native receipts no longer re-hit the API on every flush — a durable backoff gate prevents retry storms after transient failures while replays preserve the original receipt.
...
- **Sidebar App icons and recency ordering**: the Apps list now renders icon + title + slug instead of bare monospace identifiers, adds a reusable `AppIcon` with lettered fallback and bounded retry across the expanded list, collapsed flyout, active row, and drag ghost, and serves apps newest-updated-first so an edit floats back to the top. Backed by a new `v2_idx_apps_space_updated` index (migration 0066) that turns the list into a pure index scan, with realtime snapshots re-sorted to avoid pinning stale positions.
- **Unified upload limits and quota**: every durable upload path (space files, public files, chat/app attachments, avatars, gateway attachments) now shares one set of caps from `@cohub/protocol` instead of per-feature constants, and a single Redis-backed fixed-window per-user quota (5k files / 10 GiB per hour) replaces three separate rate limiters (token bucket, public-file windows, chat/avatar gates), while raising gateway single-file to 1 GiB, batch totals to 2 GiB / 2000 files, and inline uploads to 64 MiB.
- **Cross-replica relay data dials**: runner data dials landing on a gateway replica that does not own the pending peer are now forwarded over an in-cluster WebSocket to the pairing pod via a Redis-published cluster hint, eliminating the ~50% 4404/4408 pairing failures across replicas; hint publish failures degrade gracefully to prior behavior.

### Bug Fixes

- Billing access blocks are now handled as terminal, user-facing results across inbound channels: a stable de-duplicated notice with a conversion link is enqueued before the event is acknowledged instead of surfacing as a transport failure, and the billing conversion URL is exposed on `BillingConversionIntent.primaryAction.href` for clients without an in-app action.
- CLI diagnostics URL redaction no longer re-encodes healthy URLs — URLs without sensitive query keys stay byte-identical (wrapped-log trailing quotes no longer become `%22`) and the matcher no longer swallows quotes or angle brackets.
- Public assets and gateway attachments are now charged against the shared byte quota without double-counting image fallback slots.

## v2.54 — 2026-09-22

- **Local Runtime lifecycle**: `cohub runtime up` now supports supervised foreground/background execution, private local status and stop controls, persistent network recovery, redacted terminal warnings, and explicit Space reuse/new prompts. `-n` becomes the boolean alias for `--new`, `--name <name>` names a new Space, Runtime status reports file-bridge availability, and SDK runtime-status requests accept an `AbortSignal`.
- **Native Turn sync**: native Pi and Codex conversations now connect to ordinary Cohub Chats with durable per-Turn receipts, automatic whole-Turn forks on conflicting continuations, and verified native archives, reusing the existing realtime progress and continuation APIs. Adds `cohub runtime attach` / `detach`, project-scoped upload consent, capability checks, and native sync diagnostics.
- **Detached-transport execution**: losing the Runtime transport no longer aborts local model or tool work. A detached connection keeps executing, downgrades streaming to best-effort, and replays the durable result over the reconciling reconnect, so a flaky network cannot discard a Turn that already completed locally.
- **Managed Runtime control pipe**: the CLI bundles sandboxd v2.54.0, the first runner with the private managed Runtime control pipe, so `cohub runtime up` reads connection lifecycle events directly from the daemon instead of polling the API every five seconds. Older binaries stay usable through the compatibility readiness/restart path.
- **Consolidated App development reference**: App docs are unified into one bilingual, scenario-first guide with a single API-to-scope authorization map (including previously undocumented `space.completion` / `streamCompletion`), a sample index, and an owner-funded App Action example. The SDK npm package drops the duplicated `app-runtime-guide.md` and links to the docs site instead.
...
- Runtime reconnects now retake their own lease deterministically — the same runtimeId replaces a stale entry instead of waiting out the 40s TTL — and control-plane heartbeats are decoupled from lease I/O, so a slow authorize or Redis renew can no longer starve the client into a timeout.
- Relay data-channel pairing now outlives the runner's dial timeout, and sandbox dial failures distinguish timeouts from rejections; teardown-time websocket write failures drop from warn to debug.
- Process-group cleanup treats EPERM on an emptied group as the normal end state on every platform, probes Windows trees through leader liveness, and backs off snapshot polling; a cleanup failure no longer masks an already-finished native result.
- Receipt-less recovery rebuilds a completed native Turn from native transcript bytes without rewriting native data, and serve-path archive uploads resume through the Space transport again.
...
- **Event-driven Runtime recovery**: The per-minute recovery sweep and the message delivery outbox are replaced by Session-scoped reconciliation. The local Runtime reports pending executions in bounded `runtime.recovery` batches after connecting, Gateway peer disconnects and uncertain transports enqueue recovery for that Session, and stop confirmation moves into the affected Chat — more than 64 pending Sessions can no longer break a connection.
- **Completion model registry**: Model resolution is extracted into `CompletionModelRegistry`, which keeps hidden models resolvable by explicit id while excluding them from implicit defaults and provider fallbacks, fixing hidden models that had become unreachable at runtime. The model picker also reveals a hidden model when it is the current selection.
- **Runtime status probe policy**: Space runtime status now sits behind a freshness/backoff policy — a missing route is retried rarely, denials and transient errors back off, and focus/mount bursts collapse into a single request — removing needless polling without delaying the first screen.
- **App authoring contract**: `client.auth.authorize({ target, scopes })` with explicit account, Space, or Space-picker targets is now the documented consent path and reports the authoritative target, `client.desktop.reportResult()` replaces the deprecated `client.ui` alias, and the deprecated `context.space` vocabulary plus unused `work.json` manifests are gone.

### Bug Fixes

- Published HTML assets were cached as immutable, so redeploys served stale pages; HTML now revalidates with a three-day cache while other work assets stay immutable.
- The `0065` `harness_index` migration was not idempotent and could fail on re-run; it is now safe to apply repeatedly.
- Deprecated `context.space` and `client.ui` leftovers were swept from example Apps, the SDK README, and the runtime guide, whose init recipe guarded on `ctx.space.id` (empty in broker mode) and now checks `ctx.app.id`.
...
- **Generation SDK 0.1.29**: Upgraded `@neta-art/generation` across API, worker, infra, protocol, and CLI. Seedance catalog prices now show BytePlus list USD per 1M tokens (`seedance-2-0` $2.40–$7.70, `seedance-2-0-fast` $3.30–$5.60, `seedance-2-5` $6.40–$11.70), and `seedance-2-5` is added at 480p–1080p with no 4K.
- **App version history & provenance**: Published apps now expose their immutable version history — the public Cohub bar gains a version switcher (`?cohub_v=<n>`, `getBySlug(…, { version })`, `listPublicVersions()`), and every version deep-links to the session and turn that produced it. Provenance is validated against the app's own space and session identity is gated on `session.view`, so a private session never leaks; the raw publish-time `meta.source` is no longer emitted, and viewer-scoped responses are never shared-cached.
- **Source-label session filtering**: `GET /api/me/sessions?source=…` now filters by each session's source **system label** instead of the raw `space_sessions.source` string, so channel sessions are attributed by the same provider-aware labels a space sidebar shows. The per-origin `sourceCounts` aggregate was removed, dropping a full account-wide scan from every page fetch; the inbox picker lists the full source vocabulary statically.
- **Unified `@` mention search**: Composer Space mentions reuse the command palette's search and ranking (fuzzy text weighted against viewer tier and recency) instead of maintaining a second scorer, so mentions and the palette can no longer disagree about ordering — and the removed duplicate logic eliminates the per-keystroke scan over cached session lists.
- **Docked public App bar**: The public App page is now a full-height flex column with the Cohub bar (wordmark, space/app identity, views, publisher, version switcher) rendered above the App rather than floating over it, so the embedded App's viewport is exactly what the viewer sees. Version history is fetched after hydration, keeping the SSR critical path to a single request.
- **Live generation vs. queued turns**: Generation state is now resumed only from `running` / `abort_requested` turns, since a session's newest unfinished turn can be a queued follow-up waiting behind a running one. Queued turns stay in the follow-up queue, and empty pending snapshots are no longer persisted or restored.

### Bug Fixes

- Fixed `?source=` filters dropping Feishu/WeChat/Discord/QQ channel sessions and mislabeling them as `other`, caused by matching the raw source column instead of the assigned labels.
- Fixed the public app version switcher not actually switching: selection is driven by local state, rewrites `?cohub_v`, refetches the version, and restores the previous version in the URL on failure.
- Fixed the model picker omitting availability dots for generation (image/video/audio) models — the status feed now marks them in both the collapsed row and the expanded list.
- Fixed generation sessions created by direct generation missing their source system label, which previously excluded them from source-based filtering.
...
- **Command palette overview warming**: The Recent tab's overview is warmed in the background instead of revalidated on open — viewer activity (send, pin) schedules a debounced, coalesced refresh, the snapshot and its two-minute throttle are shared across tabs over the cache broadcast channel, visiting a Space no longer invalidates the snapshot, and most opens hit no network at all.
- **Generation SDK 0.1.27**: Upgraded `@neta-art/generation` across API, worker, infra, protocol, and CLI, bringing gpt-image-2 output controls — `background` (auto / opaque / transparent) and `output_format` (png / jpeg / webp) — with transparent-background examples in the generation docs.
- **Milk Frog desktop-surface example**: The `hud` and `mascot` overlay demos are consolidated into a single `milk-frog` companion App that follows the current chat, reads the live generation stream, and replies in its curator voice, demonstrating roam/dock input regions and CDN-hosted sprites.
...
- **Session detail active turn**: `spaces sessions get` and rename responses now include `activeTurn` like the session list, so clients can tell whether a turn is still running from the single-session response.
...
- **Desktop overlay surfaces**: Apps can run as a transparent, chrome-free overlay above the Space workspace instead of a preview tab — requested with `cohub desktop open <app> --as overlay` or declared at publish time via `<meta name="cohub:surface" content="overlay">`. Overlays cover the whole window and set their geometry and pointer-event regions through `cohub.app.requestConfigure()` — regions are hit-tested, so everything an overlay paints stays visible outside its interactive area and fixed panels shrink to their own size. Overlays expose `context.invocation.surface`, and keep the full App runtime (context, auth, Space APIs, realtime, `--call`, composer chips, commerce), with up to eight mounted at once.
...
- **Generation model pricing**: Model declarations can carry a display `pricing` block (`unit` plus `amount` or a `min`/`max` range), surfaced through `models.listMultimodal()` and rendered beside each model in the web model picker. The schema now lives upstream in `@neta-art/generation` 0.1.26 — `@cohub/protocol` re-exports the type, the infra declaration loader delegates validation to the SDK parser, and the duplicated local schema and its tests are gone.
...
- **Mobile viewport guard**: A fixed-height shell (`height: 100%` with `interactive-widget=resizes-content`) plus a new offset guard snaps the document back after the soft keyboard closes, so the shell no longer shifts up and exposes the root canvas; the chat list also re-pins to the tail when the viewport shrinks.
- **Gateway resilience**: QQ reconnects connect through the well-known WebSocket URL and cache `/gateway/bot` with its `session_start_limit`, backing off on frequency limits instead of hammering `GET /gateway` and misclassifying the limit as a config error, with channels reporting a `rate_limit` health reason. The gateway verifies the user access token locally before calling the API, and the sandbox relay exits with an actionable message when a token is rejected instead of retrying forever.
...
- **Overlay paint and input**: input regions only gate pointer events, so decorative parts such as bubbles, tooltips and effects render outside the interactive area; rect regions activate on hover while `"all"` works on touch. Overlay iframes stay hidden until they paint, and identical `requestConfigure` payloads are compared structurally and skipped so Apps re-sending geometry every render no longer churn the overlay list.
- **App consent**: identical in-flight `auth.request()` calls merge into one consent dialog and share its answer, instead of replacing it with a spurious denial.
...
- **Consistent browser chrome**: the PWA manifest, inline FOUC script and theme registry now share each theme's exact `--bg-primary` color, kept in lockstep by a dedicated test.
...
- **CLI board authoring workflow**: Replaced `boards nodes add/patch/remove` with `boards items create/patch/replace/delete`, added `boards examples` and `boards capabilities` for templates and schemas, and exposed the previously-hidden `tasks` and `cron-jobs` commands.
- **Realtime `board.changed` projection**: SDK board realtime events and subscriptions are now `board.changed`/`changed` with a semantic `changed` projection and no wire operations; checkpoints and published Works persist the same semantic Item snapshot as live authoring, with legacy Sequence/Node compatibility removed.
- **Batched board refreshes**: The Web preview controller now coalesces bursts of contiguous remote board events into a single authoritative snapshot reconciliation instead of issuing one read per version, cutting read amplification and render churn during high-volume updates.

### Bug Fixes

- Restore exact session scroll position when reopening a session.
- Preserve the space theme in settings and deduplicate space theme requests to avoid redundant fetches.
- Keep voice input enabled during follow-up turns in the session composer.
- Use QQ gateway intents allowed for the app so disallowed-intent (4014) closes are classified as permission errors and back off correctly.
...
- **Sandbox optimistic concurrency**: filesystem writes are now serialized per path and support versioned writes (size + mtime) that reject stale overwrites with a CONFLICT error, plus a new atomic `fs.edit` RPC that applies batched text replacements under the same lock, eliminating lost updates from concurrent tool calls.
- **Typed board generation references**: board nodes now declare typed content capabilities (text, image, video, audio, file, json, collection ports) shared across protocol and web, and the generation composer wires selected board items into generations as typed reference inputs instead of raw URLs.
- **Relative markdown asset resolution**: markdown surfaces now resolve workspace-relative image/video/audio paths into live assets with bounded concurrency, retries, and timeouts, so workspace documents render media without manual URL fixes.
- **Agent tool-call loop guard**: the agent runtime detects repeated identical tool-call batches within a turn and steers the model to stop and reassess, preventing runaway tool loops and wasted tokens.

### Bug Fixes

- Accept modern UUID versions (v6–v8) consistently across agent, API, and CLI validation via shared protocol-level identifier checks.
- Restrict publicly recordable promotion events to safe landing/ready/paywall events, blocking spoofed conversion events from unauthenticated clients.
...
- **Format-preserving avatar uploads**: Web and CLI workflows accept JPEG, PNG, GIF, and WebP avatars up to 4 MiB, with orientation-aware normalization and fallback handling when image processing fails.
- **Unified presigned PUT uploads**: The SDK now negotiates presigned PUT uploads for avatars and chat attachments, aligning public-asset uploads with direct object-storage writes.

### Bug Fixes

- **Avatar replacement reliability**: Removed cache-buster-dependent avatar URLs and fixed upload handling that could leave clients serving stale or broken images after profile or space avatar changes.
